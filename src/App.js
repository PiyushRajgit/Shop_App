...
export default function App() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 font-inter">
      <h1 className="text-4xl font-extrabold text-center mb-4 text-caribbeangreen-700">
        Item Summary Dashboard
      </h1>

      {/* Navigation Button to Sales Page */}
      <div className="text-center mb-6">
        <Link
          to="/sales"
          className="inline-block bg-caribbeangreen-400 text-white px-4 py-2 rounded font-semibold hover:bg-caribbeangreen-700 transition"
        >
          View Sales Records
        </Link>
      </div>
            ...
            sorted.forEach((item, idx) => {
              if (item.kv !== currentKV) {
                ...
                rows.push(
                  <tr
                    key={`group-${currentKV}`}
                    className="bg-yellow-300 shadow-md border border-yellow-600"
                  >
                    <td
                      colSpan="4"
                      className="text-left px-6 py-3 font-extrabold text-xl text-gray-900 tracking-wide uppercase"
